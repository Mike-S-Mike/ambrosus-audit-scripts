...
const runGasBenchmark = async () => {
  ...
  await setTimestamp(now + 2678400 * 36);
  gasUsed = getGasUsed(await tryWithdraw(atlasInitialShelterer));
  console.log(`          Withdraw by atlas initial+12m: ${gasUsed}`);

  await setTimestamp(now);
  gasUsed = getGasUsed(await tryWithdraw(atlasInitialShelterer));
  console.log(`          Withdraw by atlas initial: ${gasUsed}`);

  let startGas = gasUsed;
  for (let index = 0; index < 12; index++) {
    await setTimestamp(now + 2678400 * index);
    gasUsed = getGasUsed(await tryWithdraw(atlasInitialShelterer));
    console.log(`  Withdraw by atlas after ${index} months: ${gasUsed}`);
  }
  console.log(`  Withdraw gas increase after a year: ${gasUsed-startGas}`);
...
const setupEnvironment = async () => {
  ...
  ({challenges, time, kycWhitelist, payouts, roles, fees, uploads} = await deploy({
    web3,
    contracts: {
      challenges: true,
      bundleStore: true,
...
const tryWithdraw = async (sheltererId) => payouts.methods.withdraw(sheltererId).send({from: sheltererId});
